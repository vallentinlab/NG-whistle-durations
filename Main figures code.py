...
#------------------------------------------------------------------------------------------------------------------------------
birds_phase1 = ['pepsi', 'clipper', 'georgew', 'kalimero',
                'leo', 'mate', 'chonksMcGee']

birds_phase2 = ['pepsi', 'clipper', 'georgew', 'hope',
                'grape', 'chonksMcGee', 'gregory', 'matchbox']


stim_filtered = stim_info[
    ((stim_info['phase'] == 1) & stim_info['bird'].isin(birds_phase1)) |
    ((stim_info['phase'] == 2) & stim_info['bird'].isin(birds_phase2))
]
...
songs_filtered = birds_songs_info[
    ((birds_songs_info['phase'] == 1) & birds_songs_info['bird'].isin(birds_phase1)) |
    ((birds_songs_info['phase'] == 2) & birds_songs_info['bird'].isin(birds_phase2))
]

positive_counts = (
    songs_filtered[songs_filtered['stim_response'] == '+']
    .groupby(['bird', 'phase'])
    .size()
)

combined = pd.DataFrame({
    'n_playbacks': stim_counts,
    'n_positive': positive_counts,
})

combined['n_positive'] = combined['n_positive'].fillna(0).astype(int)
combined['n_playbacks'] = combined['n_playbacks'].astype(int)

combined['percent_positive'] = (
    combined['n_positive'] / combined['n_playbacks'] * 100
)

plot_df = combined.reset_index()[['bird', 'phase', 'percent_positive']]
...
plt.ylabel('Whistle songs (%)', fontsize=12)
plt.legend([], [], frameon=False)

plt.tight_layout()
mpl.rcParams['pdf.fonttype'] = 42
fig.savefig('Plots/whistles_percentages.pdf', transparent=True)
plt.show()

summary = plot_df.groupby('phase')['percent_positive'].describe()
print("\nNumerical summary of boxplot values:\n")
print(summary)
#------------------------------------------------------------------------------------------------------------------------------
#Data shuffling----------------------------------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------------------------------
